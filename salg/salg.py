...
class SALG:
    ...
    def __init__(self, img_h, img_w,
                 col_gap_ratio=0.06, vert_gap_ratio=0.025,
                 caption_radius=0.20, caption_iou_x=0.30,
                 float_merge_gap=0.01, nms_iou_thresh=0.60, min_conf=0.20):
        self.img_h           = img_h
        self.img_w           = img_w
    ...
    def _cluster_flow(self, dets):
        ...
        for det in sorted_dets[1:]:
            ...
            is_heading = (det.cls_name  in self.HEADING_CLASSES or
                          prev.cls_name in self.HEADING_CLASSES)
    ...
    def group(self, detections) -> List[SemanticGroup]:
        ...
        for col in range(n_cols):
            ...
            for cluster in self._cluster_flow(col_dets):
                ...
                gtype = ('title'   if 'Title'          in types else
                         'section' if 'Section-header' in types else 'flow')
        ...
        for f in float_dets:
            elems = [f] + [caption_dets[ci]
                           for ci, anch in cap_anchors.items() if anch is f]
